...
service cloud.firestore {
  match /databases/{database}/documents {
        ...
        return request.query.limit <= 50 &&
          (
            ...
            || (
              ...
              request.query.where.value.all(type, type in validTypes)
            )
          );
      }
    ...
    match /projectEvents/{eventId} {
      ...
      function validateProjectEvent() {
        ...
        return projectExists &&
          ...
          (
            ...
            data.reactions.comments.all(comment,
              comment is map &&
          ...
          (
            ...
            data.attendees.all(uid, uid is string)
          );
      }
      ...
      function validateProjectEventUpdate() {
        ...
        return immutableFieldsUnchanged &&
          (
            (reactionsValid && data.attendees == prevData.attendees) ||
            (attendeesValid && data.reactions == prevData.reactions)
          );
      }
    }
    ...
    match /users/{userId} {
      ...
      allow update: if (isOwnUser(userId) && validateUserUpdate()) || isAdmin();
      allow delete: if isAdmin();
      ...
      match /snoozed/{symbol} {
        ...
        function validateSnoozed() {
          ...
          return data.expiry is number &&
            data.expiry > request.time.millis();
        }
      }
      ...
      function validateUser() {
        ...
        return (
          ...
          data.email.size() > 0 &&
          (data.username == null || isValidUsername(data.username, 3, 50)) &&
          ...
          (data.roles == null || (
            data.roles is map &&
            (data.roles.admin == null || data.roles.admin is bool) &&
            (data.roles.moderator == null || data.roles.moderator is bool)
          )) &&
      ...
      function validateUserUpdate() {
        ...
        return data.email is string &&
          ...
          (data.roles == null || (
            data.roles is map &&
            (data.roles.admin == null || data.roles.admin == prevData.roles.admin) &&
            (data.roles.moderator == null || data.roles.moderator == prevData.roles.moderator)
          )) &&
          (data.hasSeenTutorial == null || data.hasSeenTutorial is bool) &&
          (data.uid == null || data.uid == prevData.uid) &&
          (data.preferences == null || (
            data.preferences is map &&
            (data.preferences.favorites == null || (
              data.preferences.favorites is list &&
              data.preferences.favorites.all(addr, isValidAddress(addr))
            )) &&
            (data.preferences.notificationFilter == null || (
              data.preferences.notificationFilter is map &&
              (data.preferences.notificationFilter.excludeTypes == null || (
                data.preferences.notificationFilter.excludeTypes is list &&
                data.preferences.notificationFilter.excludeTypes.all(type, type is string)
              )) &&
              (data.preferences.notificationFilter.type == null || data.preferences.notificationFilter.type is string)
            )) &&
            (data.preferences.notifications == null || (
              data.preferences.notifications is map &&
              (data.preferences.notifications.ai_index == null || data.preferences.notifications.ai_index is bool) &&
              (data.preferences.notifications.eth_stats == null || data.preferences.notifications.eth_stats is bool) &&
              (data.preferences.notifications.loser == null || data.preferences.notifications.loser is bool) &&
              (data.preferences.notifications.mover == null || data.preferences.notifications.mover is bool) &&
              (data.preferences.notifications.new_token == null || data.preferences.notifications.new_token is bool) &&
              (data.preferences.notifications.news == null || data.preferences.notifications.news is bool) &&
              (data.preferences.notifications.price_spike == null || data.preferences.notifications.price_spike is bool) &&
              (data.preferences.notifications.volume_spike == null || data.preferences.notifications.volume_spike is bool)
            )) &&
            (data.preferences.prioritizeNotifications == null || data.preferences.prioritizeNotifications is bool) &&
            (data.preferences.terminalStyle == null || isValidString(data.preferences.terminalStyle, 1, 50))
          ));
      }
    }
...
service firebase.storage {
  match /b/{bucket}/o {
    // Thumbnails: Public read, admin write (≤2MB, images only)
    ...
    // Token Logos: Public read, authenticated write (≤2MB, images only)
    match /token-logos/{imageId} {
      allow read: if true;
      allow write: if isAuthenticated() &&
        request.resource.size < 2 * 1024 * 1024 &&
        request.resource.contentType.matches('image/(jpeg|png|gif|webp)') &&
        exists(/databases/$(database)/documents/pendingTokens/$(imageId)) &&
        get(/databases/$(database)/documents/pendingTokens/$(imageId)).data.submittedBy == request.auth.uid;
    }

    // Cypherscope Token Images: Public read, admin write (≤2MB, images only)
    match /cypherscope-images/{imageId} {
      allow read: if true;
      allow write: if isAdmin() &&
        request.resource.size < 2 * 1024 * 1024 &&
        request.resource.contentType.matches('image/(jpeg|png|gif|webp)');
    ...
    // Profile Pictures: Public read, user write (≤2MB, images only)
    ...
    // Homescan Data: Public read, admin upload (≤5MB, images or PDF)
    match /homescan/{allPaths=**} {
      allow read: if true;
      allow write: if isAdmin() &&
    ...
    // Default rule: Authenticated read, admin write
    match /{allPaths=**} {
      allow read: if isAuthenticated();
      allow write: if isAdmin();
    }
